...
import { FIELD_SIZE_BIGINT, getDefaultLeaf, getPublicKey, getUtxoType } from "./lib/utxo.helper";
import { UtxoCoinInfo, UtxoPayableCoin } from "./lib/utxo/coin";
import { Utxo } from "../typechain-types";
...
export async function genTxForZeroIn(tree: IncrementalQuinTree, outAmts: bigint[]) {
  ...
  const publicInfo: Utxo.PublicInfoStruct = {
  ...
  writeFileSync(inputPath, JSON.stringify(toDecimalStringObject(circuitInput), null, 2));
  console.log({
  ...
  const utxoData: Utxo.UtxoDataStruct = {
    proof: {
      a: calldata[0],
      b: calldata[1],
      c: calldata[2],
...
function toPublicInfoHash(publicInfo: Utxo.PublicInfoStruct) {
  const data = utils.defaultAbiCoder.encode(
    ["tuple(bytes2,address,address,uint256,bytes)"],
    [
      [publicInfo.utxoType, publicInfo.recipient, publicInfo.relayer, publicInfo.fee, publicInfo.data]
    ]
  )
